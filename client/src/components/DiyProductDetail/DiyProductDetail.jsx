...
import productCatalogDiy from "../data/productCatalogDiys";
import "swiper/css";
...
export default function DiyProductDetail() {
  ...
  const [activeProductIndex, setActiveProductIndex] = useState(() => 
    Math.max(0, productCatalogDiy.findIndex(p => p.id === Number(id)))
  );
  const [selectedImageIndices, setSelectedImageIndices] = useState(() => 
    productCatalogDiy.map(() => 0)
  );
  ...
  const currentProduct = useMemo(() => 
    productCatalogDiy[activeProductIndex], [activeProductIndex]
  );
  ...
  const handleSlideChange = useCallback(async (swiper) => {
    ...
    updateUrl(productCatalogDiy[newIndex].id, selectedImageIndices[newIndex]);
 ...
 useEffect(() => {
  const interval = setInterval(() => {
    ...
    setSelectedImageIndices((prevIndices) => {
      ...
      const product = productCatalogDiy[activeProductIndex];
...
<div className=" block md:hidden w-[100%] mt-7 ">
    ...
    slidesPerView="auto"
        ...
        resistanceRatio={0}
      >
        {productCatalogDiy.map((product, index) => (
